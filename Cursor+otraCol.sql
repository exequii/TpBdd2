																			...
																			BEGIN
																				...
																				ELSE
																					BEGIN
																						...
																						-- ALTER TABLE @bdName.@tableName ADD @columnName VARCHAR(50) NULL;
																						...
																						select @tipo = DATA_TYPE from information_schema.columns WHERE TABLE_NAME=@tabla AND COLUMN_NAME=@Nombre_Columna2

																						Set @tabla = 'Test.' + @Nombre_Tabla
																						Set @Bd= @BaseOrigen;
																						if(@tipo = 'varchar')
																							Set @query2 = 'Alter table ' + @Bd + '.' + @tabla + 'add' + @Nombre_Columna2 +' ' + @tipo + '(50) null';
																						else
																							Set @query2 = 'Alter table ' + @Bd + '.' + @tabla + 'add' + @Nombre_Columna2 +' ' + @tipo + 'null';
...
/*create proc alter_table(@tableName varchar(50),@columnName varchar(50),@bdName varchar(max))
as 
ALTER TABLE @bdName.@tableName 
ADD @columnName VARCHAR(50) NULL;
go*/


Select * from information_schema.columns WHERE TABLE_NAME='Dueño' AND COLUMN_NAME='Nombre'