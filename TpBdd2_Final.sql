...
EXECUTE SP_EXECUTESQL @CursorCampos;
	OPEN CursorCampos;
		...
		IF (@Longitud IS NULL)
			BEGIN
				 IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_NAME = 'PK_'+@Nombre_Campo)
...
END
	...
	BEGIN CATCH
		...
		OPEN CursorColumnas;
			...
			WHILE @@FETCH_STATUS = 0 
			BEGIN
				IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_NAME = 'PK_'+@Nombre_Columna)
					SET @CrearTabla = '[' + @Nombre_Columna + '] [' + @Tipo_Columna + '] PRIMARY KEY IDENTITY(1,1)';
				ELSE
