...
EXECUTE SP_EXECUTESQL @CursorCampos;
	OPEN CursorCampos;
		...
		IF (@Longitud IS NULL)
			BEGIN
				 IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @Nombre_Tabla AND CONSTRAINT_NAME LIKE ('%'+@Nombre_Campo+'%'))
					BEGIN
						IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @Nombre_Tabla AND CONSTRAINT_NAME LIKE ('PK_'+@Nombre_Campo+'%'))
							BEGIN
								PRINT('ALTER TABLE ' + @QueryBaseDestino + 'ADD ' + @Nombre_Campo + ' ' + @Tipo_Campo + ' PRIMARY KEY IDENTITY(1,1)')
							END
					END
				ELSE
			...
			BEGIN
				IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @Nombre_Tabla AND CONSTRAINT_NAME LIKE ('%'+@Nombre_Campo+'%'))
					BEGIN
						IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @Nombre_Tabla AND CONSTRAINT_NAME LIKE ('PK_'+@Nombre_Campo+'%'))
							BEGIN
								PRINT( 'ALTER TABLE ' + @QueryBaseDestino + ' ADD ' + @Nombre_Campo + ' ' + @Tipo_Campo + ' ( ' + @Longitud + ' ) PRIMARY KEY IDENTITY(1,1)')
							END
					END
				ELSE
...
SELECT * FROM DB_BaseDestino.INFORMATION_SCHEMA.COLUMNS;
 ...
 DECLARE @campotabla varchar(50)='PersonaID'
  DECLARE @tabla varchar(50)='Dueño'
 IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @tabla AND CONSTRAINT_NAME LIKE ('%'+@campotabla+'%'))
	BEGIN
		IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = @tabla AND CONSTRAINT_NAME LIKE ('PK_'+@campotabla+'%'))
			BEGIN
				PRINT(' PRIMARY KEY IDENTITY(1,1)')
			END
	END

	 IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = 'Dueño' AND CONSTRAINT_NAME LIKE ('%PersonaID%'))
	BEGIN
		IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = 'Dueño' AND CONSTRAINT_NAME LIKE ('PK_PersonaID%'))
			BEGIN
				PRINT(' PRIMARY KEY IDENTITY(1,1)')
			END
	END
